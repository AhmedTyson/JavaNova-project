document.addEventListener("DOMContentLoaded", function () {
    ...
    const updateUI = debounce(() => {
        if (currentSlideDisplay) currentSlideDisplay.textContent = currentSlideIndex + 1;
        if (progressBar) {
    ...
    function goToSlide(index, behavior = "smooth") {
        ...
        if (target) target.scrollIntoView({ behavior, block: "center", inline: "nearest" });
        updateUI();
    ...
    document.addEventListener("keydown", e => {
        ...
        switch (e.key) {
            case "ArrowUp":
            case "PageUp":
                e.preventDefault(); goToSlide(Math.max(currentSlideIndex - 1, 0));
                break;
            case "ArrowDown":
            case "PageDown":
            case " ":
                e.preventDefault(); goToSlide(Math.min(currentSlideIndex + 1, totalSlides - 1));
                break;
            case "Home":
    ...
    const swipeThreshold = 50, maxSwipeTime = 300;
    ...
    document.addEventListener("touchend", e => {
        ...
        if (dt < maxSwipeTime && Math.abs(dist) > swipeThreshold) {
            if (dist > 0) goToSlide(Math.min(currentSlideIndex + 1, totalSlides - 1));
    ...
    function detectFullscreenMode() {
        ...
        const fh = window.innerHeight, fw = window.innerWidth;
        const sh = screen.height, sw = screen.width;
        const prev = isF11;
        isF11 = (fh >= sh - 30 && fw >= sw - 30 && !apiFS);
        if (isF11 !== prev) {
            document.body.classList.toggle("f11-detected", isF11);
            if (isF11) optimizeForFullscreen(); else resetFullscreenOptimizations();
        }
    }
    function optimizeForFullscreen() {
        ...
        document.querySelectorAll(".cosmic-background").forEach(b => b.style.willChange = "transform");
    }
    function resetFullscreenOptimizations() {
        ...
        document.querySelectorAll(".cosmic-background").forEach(b => b.style.willChange = "auto");
    }
        ...
        entries.forEach(ent => {
            if (ent.isIntersecting) {
                [
                    ...
                    if (elem) setTimeout(() => elem.style.height = bar.height, bar.delay);
                });
            } else {
                ["bar-students","bar-success","bar-months","bar-jobs"].forEach(id => {
    ...
    function initialize() {
        ...
        ["fullscreenchange","webkitfullscreenchange","mozfullscreenchange","MSFullscreenChange"]
            .forEach(evt => document.addEventListener(evt, detectFullscreenMode, { passive: true }));
        document.addEventListener("keydown", e => {
            if (e.key === "F11") setTimeout(detectFullscreenMode, 100);
        }, { passive: true });
        console.log("✅ JavaNova Presentation optimized and initialized successfully");
    }
